...
class TC(testcase.TestCase):
    ...
    def test_01_all_testfiles(self):
        ...
        command = '{valgrind} {valgrind_args} {clamscan} -d {path_db} {testfiles}'.format(
            ...
            path_db=TC.path_db / 'huntermatrix.hdb',
            testfiles=testfiles,
        )
        output = self.execute_command(command)
    ...
    def test_02_all_testfiles_ign2(self):
        self.step_name('Test that clamscan ignores HunterMatrix-Test-File alerts')
        ...
        (TC.path_db / 'huntermatrix.ign2').write_text('HunterMatrix-Test-File\n')
        ...
        unexpected_results = ['{}: HunterMatrix-Test-File.UNOFFICIAL FOUND'.format(testpath.name) for testpath in TC.testpaths]
