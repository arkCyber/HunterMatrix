...
class TC(testcase.TestCase):
    ...
    def setUpClass(cls):
        ...
        TC.testpaths = list(TC.path_build.glob('unit_tests/input/huntermatrix_hdb_scanfiles/clam*')) # A list of Path()'s of each of our generated test files
        ...
        shutil.copy(
            str(TC.path_build / 'unit_tests' / 'input' / 'huntermatrix.hdb'),
            str(TC.path_db),
        )
        shutil.copy(
        ...
        with (TC.path_build / 'huntermatrix-config.h').open('r') as huntermatrix_config:
            if "#define HAVE_FD_PASSING 1" in huntermatrix_config.read():
    ...
    def test_clamd_00_version(self):
        ...
        expected_results = [
            'HunterMatrix {}'.format(TC.version),
        ]
    ...
    def test_clamd_02_clamdscan_version(self):
        ...
        self.verify_output(output.out,
            expected=['HunterMatrix {}'.format(TC.version)], unexpected=['Could not connect to clamd'])
    ...
    def test_clamd_03_reload(self):
        ...
        (TC.path_tmp / 'reload-testfile').write_bytes(b'HunterMatrix-RELOAD-Test')

        self.run_clamdscan('{}'.format(TC.path_tmp / 'reload-testfile'),
            expected_ec=0, expected_out=['reload-testfile: OK', 'Infected files: 0'])

        (TC.path_db / 'reload-test.ndb').write_text('HunterMatrix-RELOAD-TestFile:0:0:436c616d41562d52454c4f41442d54657374')
        ...
        self.run_clamdscan('{}'.format(TC.path_tmp / 'reload-testfile'),
            expected_ec=1, expected_out=['HunterMatrix-RELOAD-TestFile.UNOFFICIAL FOUND', 'Infected files: 1'])

    def test_clamd_04_all_testfiles(self):
        '''
        Verify that clamd + clamdscan detect each of our <build>/unit_tests/input/huntermatrix_hdb_scanfiles/clam* test files.
        '''
        ...
        expected_results = ['{}: HunterMatrix-Test-File.UNOFFICIAL FOUND'.format(testpath.name) for testpath in TC.testpaths]
    ...
    def test_clamd_06_HeuristicScanPrecedence_off(self):
        ...
        self.run_clamdscan('{}'.format(TC.path_build / 'unit_tests' / 'input' / 'huntermatrix_hdb_scanfiles' / 'clam.exe_and_mail.tar.gz'),
            expected_ec=1, expected_out=['HunterMatrix-Test-File'])
    ...
    def test_clamd_08_VirusEvent(self):
        ...
        self.run_clamdscan_file_only('{}'.format(TC.path_build / 'unit_tests' / 'input' / 'huntermatrix_hdb_scanfiles' / 'clam.exe'),
            expected_ec=1)#, expected_out=['Virus found: HunterMatrix-Test-File.UNOFFICIAL'])
        ...
        self.verify_log(str(TC.path_tmp / 'test-clamd.log'),
            expected=['Virus found: HunterMatrix-Test-File.UNOFFICIAL'],
    ...
    def test_clamd_09_clamdscan_ExcludePath(self):
        ...
        shutil.copy(str(TC.path_build / 'unit_tests' / 'input' / 'huntermatrix_hdb_scanfiles' / 'clam.exe'), str(TC.path_tmp / 'alpha' / 'a_found'))     # This should be found (first)
        shutil.copy(str(TC.path_build / 'unit_tests' / 'input' / 'huntermatrix_hdb_scanfiles' / 'clam.exe'), str(TC.path_tmp / 'beta' / 'b_excluded'))  # This one should be excluded
        shutil.copy(str(TC.path_build / 'unit_tests' / 'input' / 'huntermatrix_hdb_scanfiles' / 'clam.exe'), str(TC.path_tmp / 'charlie' / 'c_found'))     # This one should still be found after excluding the previous
        ...
        expected_out = [
            'a_found: HunterMatrix-Test-File.UNOFFICIAL FOUND',
          # 'b_excluded: Excluded',  <-- Bug: this doesn't appear in recursive regular scans :-(, only fdpass and stream
            'c_found: HunterMatrix-Test-File.UNOFFICIAL FOUND',
        ]

        unexpected_out = [
            'a_found: Excluded',
            'b_excluded: HunterMatrix-Test-File.UNOFFICIAL FOUND',
            'c_found: Excluded',
        ]
    ...
    def test_clamd_12_onenote_disabled(self):
        ...
        testpaths = [
            TC.path_build / "unit_tests" / "input" / "huntermatrix_hdb_scanfiles" / "clam.exe.2007.one",
            TC.path_build / "unit_tests" / "input" / "huntermatrix_hdb_scanfiles" / "clam.exe.2010.one",
            TC.path_build / "unit_tests" / "input" / "huntermatrix_hdb_scanfiles" / "clam.exe.webapp-export.one",
        ]
        ...
        expected_results = ['{}: HunterMatrix-Test-File.UNOFFICIAL FOUND'.format(testpath.name) for testpath in testpaths]
