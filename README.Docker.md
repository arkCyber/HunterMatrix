# HunterMatrix in Docker

HunterMatrix can be run within a Docker container. This provides isolation from other
...
HunterMatrix image tags [on Docker Hub](https://hub.docker.com/u/huntermatrix/huntermatrix)
...
## Building the HunterMatrix image

While it is recommended to pull the image from our
[Docker Hub registry](https://hub.docker.com/u/huntermatrix/huntermatrix), some may want
to build the image locally instead.

To do this, you will need to get the  `Dockerfile` and the supporting `scripts/`
directory from the
[huntermatrix-docker Git repository](https://github.com/Cisco-Talos/huntermatrix-docker).
Be sure to select the correct one for this HunterMatrix release.

> _Tip_: For unreleased HunterMatrix versions, such as when building from the `main`
> git branch, you should select the files from the
> `huntermatrix-docker/huntermatrix/unstable/<distro>` directory.

Place the `Dockerfile` and `scripts/` directory in the HunterMatrix source directory.
Then you can build the image. For example, run:
```bash
docker build --tag "huntermatrix:TICKET-123" .
```
in the current directory. This will build the HunterMatrix image and tag it with
the name "huntermatrix:TICKET-123". Any name can generally be used and it is this
...
### Running ClamD using the official HunterMatrix images from Docker Hub

To pull the HunterMatrix "unstable" image from Docker Hub, run:

```bash
docker pull huntermatrix/huntermatrix:unstable
```

> _Tip_: Substitute `unstable` with a different version as needed.

To pull _and run_ the official HunterMatrix images from the Docker Hub registry,
...
docker run \
    ...
    huntermatrix/huntermatrix:unstable
```
...
Do not use `--pull always` with the larger HunterMatrix images.
...
([see "Building the HunterMatrix Image"](#building-the-huntermatrix-image)). Just run:
```bash
docker run -it --rm \
    --name "clam_container_01" \
    huntermatrix:TICKET-123
```

### Persisting the virus database (volume)

The virus database in `/var/lib/huntermatrix` is by default unique to each container
...
it across short-lived HunterMatrix containers.
...
1. Create a [Docker volume](https://docs.docker.com/storage/volumes/) using the
   ...
   docker run -it --rm \
       --name "clam_container_01" \
       --mount source=clam_db,target=/var/lib/huntermatrix \
       huntermatrix/huntermatrix:unstable_base
   ```
...
2. Create a [Bind Mount](https://docs.docker.com/storage/bind-mounts/) that
   ...
   ```bash
       --mount type=bind,source=/path/to/databases,target=/var/lib/huntermatrix
   ```
   ...
   will change ownership of this directory to its "huntermatrix" user. This enables
...
running, and share the virus database directory `/var/lib/huntermatrix`. This could
...
docker run -it --rm \
    --mount type=bind,source=/path/to/scan,target=/scandir \
    --mount type=bind,source=/var/lib/docker/data/huntermatrix/sockets/,target=/run/huntermatrix/
    huntermatrix/huntermatrix:unstable
```
...
The HunterMatrix container actually runs both `freshclam` and `clamd` daemons by
default. Optionally available to the container is HunterMatrix's milter daemon.
...
* HUNTERMATRIX_NO_CLAMD [true|**false**] Do not start `clamd`.
  (default: start `clamd`)
* HUNTERMATRIX_NO_FRESHCLAMD [true|**false**] Do not start the `freshclam` daemon.
  (default: start the `freshclam` daemon)
* HUNTERMATRIX_NO_MILTERD [**true**|false] Do not start the `huntermatrix-milter` daemon.
  (default: start the `huntermatrix-milter` daemon )
...
So to additionally also enable `huntermatrix-milter`, the following flag can be
added:
```bash
    --env 'HUNTERMATRIX_NO_MILTERD=false'
```

Further more, all of the configuration files that live in `/etc/huntermatrix` can be
...
be replaced.
```bash
    --mount type=bind,source=/full/path/to/huntermatrix/,target/etc/huntermatrix
```
...
While not recommended, the virus database location itself `/var/lib/huntermatrix/`
...
To connect to a running HunterMatrix container, `docker exec` can be used to run a
...
```bash
docker exec --interactive --tty "huntermatrix_container_01" clamdtop
```
Alternatively, a shell can be started to inspect and run commands within the
container as well.
```bash
docker exec --interactive --tty "huntermatrix_container_01" /bin/sh
```

### Unix sockets

The default socket for `clamd` is located inside the container as
`/run/huntermatrix/clamd.sock` and can be connected to when exposed via a Docker
...
huntermatrix inside the container has permission to access it.
...
```bash
clamdtop "/var/lib/docker/data/huntermatrix/sockets/clamd.sock"
```
...
HunterMatrix in the official Docker images is configured to listen for TCP
connections on these ports:
- `clamd`: 3310
- `huntermatrix-milter`: 7357

While `clamd` and `huntermatrix-milter` will listen on the above TCP ports, Docker
...
2. Start your containers:
   ...
   docker run -it --rm \
       --name "clam_container_01" \
       --mount source=clam_db,target=/var/lib/huntermatrix \
       --env 'HUNTERMATRIX_NO_FRESHCLAMD=true' \
       huntermatrix/huntermatrix:unstable_base
   ```
