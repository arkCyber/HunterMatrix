# Installing HunterMatrix

See our online documentation for installation instructions:
- [Installing HunterMatrix Main Page](https://docs.huntermatrix.net/manual/Installing.html)
- [Third-Party Package Installation](https://docs.huntermatrix.net/manual/Installing/Packages.html)

**For step-by-step compiling instructions** for each major operating system
and distribution, see:
- [Unix/Linux/Mac](https://docs.huntermatrix.net/manual/Installing/Installing-from-source-Unix.html)
- [Windows](https://docs.huntermatrix.net/manual/Installing/Installing-from-source-Windows.html)

You can find additional tips for development builds in our
[online documentation](https://docs.huntermatrix.net/manual/Development/development-builds.html).
...
- The test suite will fail to run if you have `pytest` from Python2 installed
  ...
  recompiling huntermatrix and trying again.
...
- [Dependencies - Unix/Linux/Mac ](https://docs.huntermatrix.net/manual/Installing/Installing-from-source-Unix.html#install-prerequisites)

- [Dependencies - Windows](https://docs.huntermatrix.net/manual/Installing/Installing-from-source-Windows.html#building-the-library-dependencies)
...
#### libhuntermatrix dependencies

App developers that only need libhuntermatrix can use the `-D ENABLE_LIBHUNTERMATRIX_ONLY`
option to bypass the libfreshclam and program dependencies.

libhuntermatrix requires these library dependencies:
...
If you are building an app and need libhuntermatrix _and_ libfreshclam but don't need
to build the HunterMatrix programs, configure the build with `-D ENABLE_APP=OFF`.
...
For regular folk who want the HunterMatrix apps, you'll also need:
...
- headers in `include`,
- configs in `etc`,
- and databases in `share/huntermatrix`.
...
- `CVD_CERTS_DIRECTORY`: Customize the HunterMatrix CA certificates directory, may be relative.
- `SYSTEMD_UNIT_DIR`: Install SystemD service files to a specific directory.

This example configuration should be familiar if you've used the HunterMatrix
...
cmake .. \
    ...
    -D APP_CONFIG_DIRECTORY=/etc/huntermatrix \
    -D CVD_CERTS_DIRECTORY=/etc/huntermatrix/certs \
    -D DATABASE_DIRECTORY=/var/lib/huntermatrix \
...
HunterMatrix has a couple other important paths you can configure. At this time,
...
- `LocalSocket`: You may configure ClamD to listen on a TCP socket or on a
  ...
  a directory like `/var/run/huntermatrix`.

- `TemporaryDirectory`: HunterMatrix creates a lot of temp files when scanning.
  By default, ClamD and ClamScan will use the system's default temp directory,
  which is typically `/tmp` or `/var/tmp`. But it may be best to give HunterMatrix
  its own directory. Maybe `/var/lib/huntermatrix-tmp`.
...
If you're building with `ENABLE_LIBHUNTERMATRIX_ONLY=ON` or `ENABLE_APP=OFF`, then
`libcheck` will still be required and you can still run the tests, but it will
skip all app tests and only run the libhuntermatrix unit tests.

To run the tests, first build HunterMatrix, then run `ctest`.
...
[on GitHub Issues](https://github.com/Cisco-Talos/huntermatrix/issues).
...
The following is a complete list of CMake options unique to configuring HunterMatrix:
...
- `DATABASE_DIRECTORY`: Database directory.
  ...
  _Default: Windows: `database`, POSIX: `share/huntermatrix`_

- `CVD_CERTS_DIRECTORY`: HunterMatrix CA certificates directory.
  Relative to the `CMAKE_INSTALL_PREFIX` unless an absolute path is given.

  _Default: Windows: `certs`, POSIX: `etc/certs`_

- `HUNTERMATRIX_USER`: HunterMatrix User (POSIX-only).

  _Default: `huntermatrix`_

- `HUNTERMATRIX_GROUP`: HunterMatrix Group (POSIX-only).
...
- `ENABLE_ALL_THE_WARNINGS`: By default we use `-Wall -Wextra -Wformat-security`
  for HunterMatrix libraries and programs. This option enables a whole lot more.
...
- `ENABLE_JSON_SHARED`: Prefer linking with libjson-c shared library instead of
  ...
  **Important**: Please set this to `OFF` if you're an application developer
  that uses a different JSON library in your app _OR_ if you provide libhuntermatrix
  that others may use in their apps. If you link libhuntermatrix with the json-c
...
- `ENABLE_APP`: Build the HunterMatrix programs (clamscan, clamd, clamdscan, sigtool,
...
- `ENABLE_MILTER`: (Posix-only) Build the huntermatrix-milter Sendmail filter daemon.
  Requires: `ENABLE_APP`
...
- `ENABLE_DOXYGEN`: Generate Doxygen HTML documentation for `huntermatrix.h`,
...
- `ENABLE_LIBHUNTERMATRIX_ONLY`: Build libhuntermatrix only.

  > _Tip_: This Excludes libfreshclam too! Use `ENABLE_APP=OFF` instead if
  > you want libhuntermatrix and libfreshclam.

  _Default: `OFF`_

- `ENABLE_STATIC_LIB`: Build libhuntermatrix and/or libfreshclam static libraries.
...
- `ENABLE_SHARED_LIB`: Build libhuntermatrix and/or libfreshclam shared libraries.
...
- `MAINTAINER_MODE`: Generate Yara lexer and grammar C source with Flex & Bison.
  Generate Rust bindings (`libhuntermatrix_rust/src/sys.rs`).
...
HunterMatrix has two bytecode runtimes:
...
HunterMatrix currently supports LLVM versions 8.0 through 13.x.
...
```
Normally, HunterMatrix would use the `FindLLVM.cmake` module in our `<src>/cmake`
...
>   enabled for the HunterMatrix build. Using `clang` instead of `gcc` may have better
>   results.
> - Building HunterMatrix in Debug-mode with a Release-LLVM build may fail, and
>   building HunterMatrix in Release-mode with a Debug-LLVM build may fail.
> - The unit tests may fail in Debug-mode builds on the `libhuntermatrix` "bytecode"
...
Cross-compiling in HunterMatrix with CMake & Rust is experimental at this time.
...
HunterMatrix's Rust toolchain integration also complicates the build.
...
The HunterMatrix project provides a CMake option `-D RUST_COMPILER_TARGET=<triple>`
...
Step-by-step instructions for cross-compiling HunterMatrix:
...
You will find the manifest in the directory where you compiled HunterMatrix. If you
followed the recommendations (above), then you will find it at
`<huntermatrix source directory>/build/install_manifest.txt`.

Feel free to inspect the file so you're comfortable knowing what you're about
to delete.

Open a terminal and `cd` to that `<huntermatrix source directory>/build` directory.
Then run:
```bash
