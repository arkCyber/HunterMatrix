...
class TC(testcase.TestCase):
    ...
    def test_xls_jpeg_png(self):
        self.step_name('Test that huntermatrix can successfully extract jpeg and png images from XLS documents')
    ...
    def test_xls_with_detection(self):
        self.step_name('Test that huntermatrix can successfully alert on PNG image extracted from XLS documents')
        # This tests a regression wherein extracted images weren't properly scanned, or the scan result recorded.
        # Note: we aren't testing the JPEG detection because the JPEG attached to the sample XLS is not properly fuzzy-hashed by huntermatrix, yet.
    ...
    def test_HTML_style_with_detection(self):
        self.step_name('Test that huntermatrix can successfully alert on PNG image extracted from HTML <style> blocks')
        # This tests a regression wherein extracted images weren't properly scanned, or the scan result recorded.
        # Note: we aren't testing the JPEG detection because the JPEG attached to the sample XLS is not properly fuzzy-hashed by huntermatrix, yet.
        # TODO: Once it is working, add the JPEG detection test.
