...
class TC(testcase.TestCase):
    ...
    def test_container(self):
        self.step_name('Test that huntermatrix can successfully alert on jpeg image extracted from XLS documents')
        # Note: we aren't testing PNG because the attached PNG is not properly fuzzy-hashed by huntermatrix, yet.
    ...
    def test_clamscan_container_cache(self):
        ...
        if not not_eicar_file.exists():
            with not_eicar_file.open('wb') as f:
                f.write(b"HUNTERMATRIX-TEST-STRING-NOT-EICAR")
