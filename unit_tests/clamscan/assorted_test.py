...
class TC(testcase.TestCase):
    ...
    def test_weak_indicator_icon(self):
        ...
        (TC.path_tmp / 'icon.ldb').write_text(
            "HunterMatrix-Test-Icon-EA0X;Engine:52-1000,Target:1,IconGroup1:ea0x-grp1,IconGroup2:*;(0);0:4d5a\n"
            "HunterMatrix-Test-Icon-IScab;Engine:52-1000,Target:1,IconGroup2:iscab-grp2;(0);0:4d5a\n"
        )
        ...
        expected_results = [
            'clam_IScab_ext.exe: HunterMatrix-Test-Icon-IScab.UNOFFICIAL FOUND',
            'clam_IScab_int.exe: HunterMatrix-Test-Icon-IScab.UNOFFICIAL FOUND',
        ]
        if fpu_endian_output.ec == 3:
            expected_num_infected = 3
        else:
            expected_results.append('clam.ea06.exe: HunterMatrix-Test-Icon-EA0X.UNOFFICIAL FOUND')
    ...
    def test_pe_cert_trust_archive(self):
        ...
        command = '{valgrind} {valgrind_args} {clamscan} \
            ...
            broken_dbs=test_path / 'broken-sigs',
            trust_dbs=test_path / 'trust-sigs',
            huntermatrix_hdb=TC.path_source / 'unit_tests' / 'input' / 'huntermatrix.hdb',
        ...
        expected_results = [
            'trust_plus_mal.zip: HunterMatrix-Test-File.UNOFFICIAL FOUND',
            'trust_plus_mal2.zip: HunterMatrix-Test-File.UNOFFICIAL FOUND',
        ]
    ...
    def test_onenote_disabled(self):
        ...
        command = '{valgrind} {valgrind_args} {clamscan} -d {path_db} {testfiles}'.format(
            valgrind=TC.valgrind, valgrind_args=TC.valgrind_args,
            clamscan=TC.clamscan,
            path_db=TC.path_build / 'unit_tests' / 'input' / 'huntermatrix.hdb',
            testfiles=testfiles,
        )
        output = self.execute_command(command)
